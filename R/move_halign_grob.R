move_halign_grob <- function(loon.grob, index, swap, halignY, temporary = FALSE, ...) {
  obj <- character(0)
  class(obj) <- names(loon.grob$children)
  UseMethod("move_halign_grob", obj)
}

move_halign_grob.l_plot <- function(loon.grob, index, swap, halignY, temporary = FALSE, ...) {
      ...
      lapply(index,
             function(i) {
               ...
               if(grepl(newGrob$children[[i]]$name, pattern = "primitive_glyph")) {
                 ...
                 polyline_grob$x <- unit(halignY, "native") +
      ...
      lapply(index,
             function(i) {
               ...
               if(grepl(newGrob$children[[i]]$name, pattern = "primitive_glyph")) {
                 ...
                 polyline_grob$y <- unit(halignY, "native") +
...
move_halign_grob.l_graph <- function(loon.grob, index, swap, halignY, temporary = FALSE) {
           ...
           function(i) {
             ...
             if(!grepl(grobi$name, pattern = "missing")) {
               ...
               edgesGrob$children[[i]] <<- if(i %in% index) {
                 ...
                 x <- c(rep(halignY, num_line), c(grobi$x)[(num_line + 1) : (2*num_line)])
           ...
           function(i) {
             ...
             if(!grepl(grobi$name, pattern = "missing")) {
               ...
               edgesGrob$children[[i]] <<- if(i %in% index) {
                 ...
                 y <- c(rep(halignY, num_line), c(grobi$y)[(num_line + 1) : (2*num_line)])
                 change_id <- which(to_id %in% index)[which(to_id %in% index) > num_line]
