move_vdist_grob <- function(loon_grob, index, swap, vdist_x, temporary = FALSE, ...) {
  obj <- character(0)
...
move_vdist_grob.l_plot <- function(loon_grob, index, swap, vdist_x, temporary = FALSE, ...) {
             ...
             function(i) {
               ...
               if(str_detect(newGrob$children[[i]]$name, "primitive_glyph")) {
                 newGrob$children[[i]] <<- editGrob(
                   ...
                   y = unit(vdist_x[which(index %in% i)], "native")
                 )
                 ...
                 polyline_grob$y <- unit(vdist_x[which(index %in% i)], "native") + 
                 ...
                 point_grob$y <- unit(vdist_x[which(index %in% i)], "native")
                 line_grob$y <- unit(rep(vdist_x[which(index %in% i)], 2), "native")
                 ...
                 image_border_grob$y <- unit(vdist_x[which(index %in% i)], "native")
                 image_grob$y <- unit(vdist_x[which(index %in% i)], "native")
      ...
      lapply(index,
             function(i) {
               ...
               if(str_detect(newGrob$children[[i]]$name, "primitive_glyph")) {
                 newGrob$children[[i]] <<- editGrob(
                   grob = newGrob$children[[i]],
                   x = unit(vdist_x[which(index %in% i)], "native")
                 )
               } else if(str_detect(newGrob$children[[i]]$name, "serialaxes_glyph"))  {
                 ...
                 polyline_grob$x <- unit(vdist_x[which(index %in% i)], "native") + 
                 ...
                 point_grob$x <- unit(vdist_x[which(index %in% i)], "native")
                 line_grob$x <- unit(rep(vdist_x[which(index %in% i)], 2), "native")
                 ...
                 image_border_grob$x <- unit(vdist_x[which(index %in% i)], "native")
                 image_grob$x <- unit(vdist_x[which(index %in% i)], "native")
...
move_vdist_grob.l_graph <- function(loon_grob, index, swap, vdist_x, temporary = FALSE) {
  ...
  if(!temporary & swap) {
    ...
    lapply(index,
           function(i) {
             ...
             nodesGrob$children[[i]] <<- editGrob(
               ...
               y = unit(vdist_x[which(index == i)], "native")
             )
           }
    )
           ...
           function(i) {
             ...
             if(!str_detect(grobi$name, "missing")) {
               ...
               edgesGrob$children[[i]] <<- if(i %in% index) {
                 ...
                 y <- c(rep(vdist_x[which(index == i)], num_line), c(grobi$y)[(num_line + 1) : (2*num_line)])
                 change_id <- which(to_id %in% index)[which(to_id %in% index) > num_line]

                 if(length(change_id) > 0) {

                   y[change_id] <- vdist_x[which(index %in% to_id[change_id])]
                   editGrob(
                     grobi,
                     y = unit(y,"native")
                   )
    ...
    lapply(index,
           function(i) {
             ...
             nodesGrob$children[[i]] <<- editGrob(
               grob = nodesGrob$children[[i]],
               x = unit(vdist_x[which(index == i)], "native")
             )
           }
    )
           ...
           function(i) {
             ...
             if(!str_detect(grobi$name, "missing")) {
               ...
               edgesGrob$children[[i]] <<- if(i %in% index) {
                 ...
                 x <- c(rep(vdist_x[which(index == i)], num_line), c(grobi$x)[(num_line + 1) : (2*num_line)])
                 change_id <- which(to_id %in% index)[which(to_id %in% index) > num_line]

                 if(length(change_id) > 0) {

                   x[change_id] <- vdist_x[which(index %in% to_id[change_id])]
                   editGrob(
                     grobi,
                     x = unit(x,"native")
                   )
