get_bin_xy <- function(binX, binId, binwidth, yshows, color, n) {
  ...
  not_show <- lapply(1:length(binId),
                     function(i){
                       if(length(binId[[i]]) != 0) {
                         colorBinHeight <- if(yshows == "frequency") table(color[binId[[i]]]) else table(color[binId[[i]]])/(n * binwidth)
                         # reorder color bin height
                         colorBinHeight <- colorBinHeight[order(unname(sapply(names(colorBinHeight), function(name) which(colorOrder %in% name))))]
                         # cumsum
