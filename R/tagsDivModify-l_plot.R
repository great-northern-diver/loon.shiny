tagsDivModify <- function(loon.grob, tabPanelName, colorList,
...
tagsDivModify.l_plot <- function(loon.grob, tabPanelName, colorList,
                                 loonWidgetsInfo, displayedPanel) {
    ...
    remove_null(
      list(
        ...
        fixedRow(
          column(6,
                 selectizeInput(
                   paste0(tabPanelName, "modifyGlyph"),
                   label = NULL,
                   choices = if(length(loonWidgetsInfo$glyph_id) == 0) "" else c("glyph", ""),
                   selected = "glyph"
                 ),
                 offset = 2
          ),
