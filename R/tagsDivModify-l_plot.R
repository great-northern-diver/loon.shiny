tagsDivModify <- function(loon_grob, tabPanelName, 
                          loonWidgets_info) {
  obj <- character(0)
  class(obj) <- names(loon_grob$children)
  UseMethod("tagsDivModify", obj)
}

tagsDivModify.l_plot <- function(loon_grob, tabPanelName, 
                                 loonWidgets_info) {
  ...
  tags$div(
    ...
    fixedRow(
      column(6, 
             selectizeInput(
               paste0(tabPanelName, "modify_glyph"),
               label = NULL,
               choices = if(length(loonWidgets_info$glyph_id) == 0) "" else c("glyph", ""),
               selected = "glyph"
             ),
             offset = 2
      ),
